...
export function useAcceptOrder() {
  ...
  return useMutation({
    mutationFn: async (orderId: string) => {
      const { data } = await apiClient.post(ORDER_ENDPOINTS.ACCEPT(orderId));
      return data;
    },
    onSuccess: () => {
      successToast({
        description: "Order accepted — stays in Requested until payment",
      });
      invalidateOrders();
    },
    onError: (error: unknown) => {
      errorToast({
        description: getErrorMessage(error, "Failed to accept order"),
      });
    },
  });
}
...
export function useRejectOrder() {
  ...
  return useMutation({
    mutationFn: async (orderId: string) => {
      const { data } = await apiClient.post(ORDER_ENDPOINTS.REJECT(orderId));
      return data;
    },
    onSuccess: () => {
      successToast({ description: "Order rejected" });
      invalidateOrders();
    },
    onError: (error: unknown) => {
      errorToast({
